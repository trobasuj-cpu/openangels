...
export async function GET() {
  try {
    ...
    // Fetch 1200 investors (1200 to 2399)
    const p1 = supabase.from('investors_secure').select('slug, created_at').not('slug', 'is', null).range(1200, 2199);
    const p2 = supabase.from('investors_secure').select('slug, created_at').not('slug', 'is', null).range(2200, 2399);

...
export async function GET() {
  try {
    const p1 = supabase.from('investors_secure').select('slug, created_at, bio, industries, email, linkedin_url, twitter_url, website').not('slug', 'is', null).order('id').range(2400, 2999);
    const p2 = supabase.from('investors_secure').select('slug, created_at, bio, industries, email, linkedin_url, twitter_url, website').not('slug', 'is', null).order('id').range(3000, 3599);

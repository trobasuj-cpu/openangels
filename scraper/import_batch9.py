...
from investors_batch9 import INVESTORS_BATCH9
...
def run():
    print(f"\n>>> Importing {len(INVESTORS_BATCH9)} investors (batch 9 - MILESTONE)...\n")
    ok, skip, err = 0, 0, 0
    for inv in tqdm(INVESTORS_BATCH9, desc="Uploading"):
        try:
    ...
    if res.count >= 1000:
        print(">>> 1000+ INVESTORS - READY FOR PRODUCT HUNT!")
    else:

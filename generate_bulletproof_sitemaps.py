...
def create_bulletproof_sitemap(filename, offset, limit):
...
export async function GET() {{
  try {{
    ...
    const res = await fetch(`\${{supabaseUrl}}/rest/v1/investors_secure?select=slug,created_at&slug=not.is.null&order=id.asc&offset={offset}&limit={limit}`, {{
      headers: {{
        'apikey': supabaseKey,
...
create_bulletproof_sitemap('sitemap-1', 0, 1200)
create_bulletproof_sitemap('sitemap-2', 1200, 1200)
create_bulletproof_sitemap('sitemap-3', 2400, 1200)
create_bulletproof_sitemap('sitemap-4', 3600, 1400)
print("Bulletproof sitemap routes written!")
